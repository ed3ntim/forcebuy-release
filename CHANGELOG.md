...
- **An inventory can be handed in from your own browser.** Steam refuses
  inventory requests from our server's address, so open your inventory address
  while signed in to Steam, save what Steam answers, and choose that file in
  the inventory's panel. ForceBuy reads it exactly as it reads Steam's own
  answer: the same items, the same comparison, the same "this is gone now".
- **Half an inventory is refused.** A sync marks whatever it did not see as
  gone, so an incomplete file would delete the rest. Save the next pages too
  and choose them together.
- **The inventory page says why**, with the date it was measured, instead of
  letting you walk into a sync that fails.
