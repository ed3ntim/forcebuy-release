...
- **An inventory can also be read from your own browser**: open your inventory
  the way Steam returns it, save it, and choose the file in the inventory's
  panel. ForceBuy reads it exactly as it reads Steam's own answer.
- **Half an inventory is refused.** A sync marks whatever it did not see as
  gone, so an incomplete file would delete the rest. Save the following pages
  too and choose them together.
