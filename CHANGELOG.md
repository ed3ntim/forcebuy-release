...
- **A history read that was picked up where it left off could start over from
  the top.** New in the previous release and caught before anyone met it: when
  the resumed page came back empty, the search for older entries counted back
  from today rather than from the resume point, and read the whole history
  again.
