...
- **"Sold on" column and a "Sold between" filter.** The list shows when an
  item was sold or otherwise left - empty for what you still hold - and can be
  sorted by it; under "More filters" you can narrow it to a range of sale
  dates.

### Valuation

- **"Count a missing cost as 0".** A new choice under Valuation, off by
  default: every item that still has no cost after everything else - no
  receipt, nothing entered by hand, none of the drop, operation, Armory or
  trade choices - counts as having cost nothing, so its whole value counts as
  profit. Off, such items stay out of the profit figure as before.

## 0.11.3 - stickers on historic items

### Holdings

- **Stickers and charms can be saved on historic items and items in storage
  units.** Saving them was refused as "not applicable". The cause: whether an
  item can carry stickers was looked up through its catalog link, and rows
  rebuilt from the inventory history have none. It is now looked up by name,
  and a name with a wear counts as a skin.

## 0.11.2 - what a sale fetched, on the item page

### Holdings

- **An item's page shows what it cost and what it sold for.** "Origin" now
  names the purchase price next to where it came from, and a sold item gets
  two more fields: how it left and for how much (a marketplace and its price,
  or "Traded away", "Applied to an item" ...; with what arrived after the
  selling fee where that differs), and when. Before, these were only in the
  edit form, and "Acquired" carried a "Sold via" line that belonged to the sale.

## 0.11.1 - Armory pass price, a banner that stays

### Valuation

- **You enter what an Armory Pass costs, not a star.** The star is worked out
  from it: a pass gives 40 stars, so the default 15.99 USD makes a star about
  0.40 USD. The setting under Valuation is now "Price of an Armory Pass" and
  shows the star value it results in.

### Settings

- **A Save button under every section that it saves**: marketplace fees,
  valuation with the workspace defaults, and (in the desktop app) the data
  folder. The other sections save the moment you change them, as before.
- **The chart library's credit moved to "Data sources and credit"**, next to
  the other credits, from "About".

### Alerts

- **The banner about an expired Steam session stays in view while you
  scroll**, and has an X that hides it for the rest of the session. A new
  problem - another account, another key - shows again.

## 0.11.0 - historic items filter, settings in a new order, what drops cost

### Holdings

- **"Historic items" filter with Include, Exclude and Explicit.** Include (the
  default) shows them with everything else, Exclude hides them, Explicit shows
  only them. The badge on such a row now reads "Historic".
- **Status now has "Sold".** Status asks whether an item is held (Active),
  sold or missing; "Sold" is no longer offered as a location, because it is
  not a place. Saved views and links that used the old filters still work.

### Valuation

- **Drops, operation rewards and Armory items now count towards profit.**
  ForceBuy tells them apart by what Steam wrote: rank-up drops, the random
  drops after a match, souvenir drops, medals and coins are drops; mission
  rewards are an operation's up to 2022 and Armory redemptions since
  2 October 2024. Each has its own choice under Valuation: drops and operation
  rewards cost nothing, an Armory item costs its stars times the value of a
  star (0.40 USD by default, adjustable, as are the stars per kind; limited
  edition items have their own price). All three are on by default, also where
  "A drop cost nothing" was off before; switch any of them off to leave those
  items out of the profit again. Holdings shows "Operation reward" and
  "Armory" as their own origins.

### Settings

- **The setting is now called "Include historic items"**, with a description
  of what it rebuilds and a note that options for opened cases and terminals
  are coming.
- **Settings in a new order, related parts together.** Account with
  notifications; marketplace logins with the marketplaces outside Steam and the
  matches per marketplace; stored access; marketplace fees; valuation with the
  workspace defaults; data sources and credit; export and backup; the danger
  zone with deleting the account; feedback; about. Save is also at the top of
  the page now.

## 0.10.3 - historic items on the web

### Ledger

- **"Include items from before ForceBuy" now works on the web.** Switched on,
  it made no rows there at all. The cause: rows were only built once the
  inventory history counted as read to the end, and on the web that mark was
  never set, even with every event back to 2014 stored. The rows no longer wait
  for it; they appear with the next history sync, or at once when you switch
  the setting off and on again.

## 0.10.2 - drops in the dashboard, wear of stored items

### Dashboard

- **The dashboard counts profit the way Holdings does.** With "A drop cost
  nothing" or "A trade cost nothing" switched on, such items now have a cost of
  0 in dashboard widgets and in the value-over-time line too, so they count
  towards profit there. A marketplace sale is also shown after its selling fee,
  as in Holdings. Before, the dashboard valued the same items without either
  setting: a drop that counted in Holdings was missing from the dashboard's
  profit, and a CSFloat sale counted gross there and net in the list.

### Holdings

- **Items in storage units show their wear, StatTrak and Souvenir.** The
  condition was in the name ("AK-47 | Redline (Field-Tested)") but not in the
  wear column, so wear filters and wear badges missed those items - 444 of
  them on one test account. The cause: rows rebuilt from the inventory history,
  which is all a storage unit's contents are, were created without reading the
  name. They now take all three from it, and existing rows are filled in on the
  next history sync. Items from before ForceBuy get the same.

### Release

- **The desktop downloads of 0.10.0 and 0.10.1 were not published; this
  version is the first desktop release with their changes.** 0.10.1 was meant
  to fix it and stopped the same way: a test that had passed could not delete
  its temporary SQLite file on Windows in time, and the release run treated
  that as a failure. The cleanup now gives up after a few seconds instead. The
  web version was not affected.

## 0.10.1 - the 0.10.0 download

### Release

- **The desktop download of 0.10.0 was not published; this version carries the
  same changes.** A new test timed out in the release run while deleting its
  temporary export file on Windows, which stopped the desktop build. The web
  version was not affected and has run 0.10.0 since its release.

## 0.10.0 - the past in your ledger

### Ledger

- **Your ledger can include what you owned before ForceBuy.** A new setting,
  "Include items from before ForceBuy" (Settings, next to the cost basis
  choices, and in the setup), rebuilds from your inventory history the items
  that left the account before you connected it: traded away, sold on the
  Steam market or a marketplace, and stickers, patches, charms and name tags
  you applied. They appear in Holdings as sold, marked "History", with
  purchase and sale prices from your receipts where there are any, and count
  in the realised profit and the sale statistics. A filter in Holdings shows
  only these items or hides them. Off by default; turning it off removes
  them again, except a row you wrote on.
  Opened cases and keys and graffiti are not included yet: their cost needs
  the price of the day they were used, which comes later.
- **A traded-away item shows what you got for it.** On the item's page,
  "Traded for" lists what came back in that trade, linked where it has a row
  in your ledger.
- **A sticker, patch, charm or name tag you applied to an item counts as sold
  for nothing.** Its purchase price now shows as a realised loss. Before, an
  applied item left the account with no sale price and dropped out of the
  realised profit, so a sticker bought for 20 and stuck on a gun cost nothing
  in the statistics.
- **A trade-up result costs what its inputs cost.** The ten items you put into
  a trade-up contract are added up and become the cost basis of the item it
  gave you, with a note saying how many inputs had a price. Inputs you bought
  count at their purchase price; an input you got as a drop or in a trade
  counts as free only if you switched on "A drop cost nothing" or "A trade cost
  nothing". If any input has no price, the result stays without a cost rather
  than showing a sum that is too low. A cost you typed yourself is never
  replaced. Changing either of those two switches now recalculates this.

## 0.9.6 - suggestions in the catalogue search

### Search

- **The catalogue search suggests names too, and finds words in any order.**
  In the skin view the suggestions are skins without wear ("AK-47 | Redline"),
  with how many copies of any wear, StatTrak or Souvenir you hold; in the
  variant view they are the full names, as when adding a holding by hand. The
  skin view also reads a short wear as the conditions a skin comes in: "redline
  ft" finds the Redline because it exists in Field-Tested. Before, the skin
  view looked for the typed text as one piece.

## 0.9.5 - name suggestions while typing, search by words

### Search

- **Item names are suggested while you type**, with picture and how many
  copies you hold. In the holdings search the suggestions are your own items;
  in "Add manual holding" they come from the whole catalogue, your own items
  first. Arrow keys move through the list, Enter takes the highlighted name or
  searches for what you typed, Escape closes the list.
- **Search finds words in any order, and the short wear.** "redline ak ft"
  finds "AK-47 | Redline (Field-Tested)". Before, the search looked for the
  typed text as one piece, so it found nothing unless you typed the name in
  its exact order. This applies to the holdings list, the catalogue and the
  command palette. FN, MW, FT, WW and BS stand for their wear.

## 0.9.2 - a price pass without its key shows as failed

### Admin

- **A price pass without its key shows as failed.** The six-marketplace prices
  without a cs2.sh key, and the Buff163 list without a login, were listed green
  although they had fetched nothing. The cause: a missing key was reported as
  "skipped", and a skip counted as a run that worked. They now show red with
  `needs_cs2sh_key` or `needs_buff_cookie` and try again after their usual
  interval, or at once when the key is stored. The backup price lists were
  never affected: they only step aside after a run that actually fetched.

## 0.9.1 - purchase prices stay on their items, charts start as a line

### Purchase prices

- **A matched purchase stays on its item.** After a sticker or name tag went
  on, after an item came out of a storage unit, or when it arrived after its
  purchase was read, the item showed no cost although the purchase was matched.
  The cause: a purchase was written onto the row the item had at the moment of
  matching, and when the item later stood for another row only the link moved,
  not the price. The next history sync writes the missing prices back; a cost
  you typed in yourself stays.

### Charts

- **Price charts start as a line**, on item pages and on the dashboard.
  Candles and area are still one click away in the chart's toolbar.

## 0.9.0 - Doppler phases priced apart, the setup offers what the settings do

### Doppler phases

- **A Doppler or Gamma Doppler is valued at its own phase.** A Ruby, a
  Sapphire and a Phase 3 of the same knife no longer share one price. The phase
  is read from the copy's finish as Steam reports it, and the phase prices come
  with the six-marketplace pass on all six marketplaces.
- **The phase shows in the holdings list**, next to the name, and on an item's
  page next to the wear and under the float.
- **An item's page uses the phase's own prices and price history.** Until a marketplace has priced a phase, the
  copy is valued at its market name as before, and the phase label says so.
- An item ForceBuy knows only from a storage unit has no phase yet: Steam does
  not report the finish of what is stored. Case Hardened tiers are not priced
  apart.

### Item pages

- **The price history spans the whole width**, right under the key figures.
  Marketplace prices and the manual price sit on the left below it, listings,
  stickers and the timeline on the right, and the edit form comes last.
- **Charts no longer carry the TradingView logo.** The credit for the chart
  library is in Settings, under About.

### Setup and Inventories

- **The setup offers the same as the settings**: the marketplace logins with
  the same rows, all six marketplaces to value at, and locally the price feed
  key. A CSFloat key still picks CSFloat unless you chose another marketplace
  yourself.
- **On the Inventories page, too, the Buff163 warning shows only while a
  cookie is being entered.**

## 0.8.1 - marketplace logins in their own place, more marketplaces to value at

### Settings

- **Marketplace logins have their own card.** The CSFloat key and the
  Buff163 cookie each show whether one is stored, are saved on their own and
  can be revoked there. They used to be password fields among the valuation
  settings that read only "configured".
- **The Buff163 warning shows while a new cookie is being entered**, not all
  the time; saving still waits for the tick.
- **YouPin898, Skinport and SkinBaron are listed as coming soon.**
- **The ledger can be valued at YouPin898, Skinport or C5Game**, next to Steam,
  CSFloat and Buff163. Their prices come with the six-marketplace pass; an item
  without one yet is valued at another marketplace's price.
- **Removed: the CSMarketCap key**, which nothing read, and on forcebuy.net the
  field for a price feed key, which only the server's own key fills.

## 0.8.0 - six marketplaces every four hours, market indexes, a market chart

### Prices

- **Six marketplaces in one pass, every four hours**: Buff163, YouPin898,
  CSFloat, Skinport, Steam and C5Game, each with its lowest ask, highest buy
  order and number of offers, for the whole catalogue. Items you hold get a
  price reading every four hours instead of once a day, so their history fills
  in with detail from now on. Readings before this release are not filled in
  afterwards; the history grows from here.
- **C5Game is a new marketplace** in every item's price table.
- **The daily passes stay as the backup.** Each one steps aside while the new
  pass has covered its marketplace in the last twelve hours, and runs as before
  when it has not.

### Market indexes

- **Market indexes for the whole market and ten segments** - knives, gloves,
  rifles, pistols, SMGs, heavy, cases and capsules, stickers, agents, charms -
  on each of the six marketplaces. They start at 100 and move with the prices of
  the items in them every four hours, each item counting the same; items with
  fewer than three offers are left out. Add them from the widget library under
  "Market indexes"; the chart builder filters them by segment and marketplace.

### Charts

- **A new chart type, "Market chart"**: candles or a line on a real time
  axis, range (7d to all) and interval (4h, 1d, 1w) switched on the chart
  itself, further lines next to it and counts as bars underneath, with a legend
  that reads out the candle under the pointer.
- **"Value over time" is a market chart now**: the value at ask as candles,
  the value at bid dotted, what it cost dashed. A widget already on your
  dashboard keeps its old form; add it again from the library for the new one.
- **An item's price history is a market chart now**: the ask as candles, the
  bid dotted, the number of offers as bars.
- **Charts can group by hour**, for series read several times a day.

### Terms of use

- **The terms now say plainly that your Steam and marketplace accounts are
  your own risk.** Valve, Steam and the marketplaces decide on their own
  whether they restrict, ban or close an account; ForceBuy has no say in that
  and is not liable for it, nor for items, balances or sales lost with it.
- **Liability during the free test is limited to intent and gross
  negligence.** Prices and values are information from third parties, not
  advice.
- **The tick before a Steam or Buff163 cookie is stored now says** that you take
  the risk to the account as your own.

## 0.7.5 - forcebuy.net opens, a new logo, revoking marketplace logins

### forcebuy.net

- **forcebuy.net opens straight into ForceBuy.** The "coming soon" page with
  its access code is gone: anyone can sign in with Steam - you get in if you
  are on the beta list, and onto the waiting list otherwise - or try the demo.
- **When the server is restarting, forcebuy.net says so** with a short page
  instead of a Cloudflare error.

### Dashboard

- **Cases, stickers and items in storage units are no longer "Unspecified"**
  on the dashboard. Their category and rarity come from the item catalogue now.
  They came from the Steam description of an item, which items rebuilt from
  storage units never had.

### Inventories

- **Every marketplace login can be revoked from the Inventories page**: an
  inventory's own CSFloat key or Buff163 cookie, and the account-wide one it
  falls back to. Revoking asks once more, then ForceBuy forgets the login.

### Interface

- **New logo**, in the top bar, as the browser tab icon and on the page
  forcebuy.net shows while the server restarts.

## 0.7.4 - one marketplace at a time, and filters cleared with one click

### Inventories

- **CSFloat and Buff163 each have their own sync row** on the Inventories page,
  with their own "last read" and their own buttons. You can refresh one
  without waiting for the other - Buff163 in particular takes minutes.
- **"Read all" for the marketplaces reads everything again.** It sent its
  request in a form the server did not look at, so it only read what was new,
  like the other button.

### Filters

- **Every filter can be cleared with the small "x" inside it**: search fields,
  lists, multiple choice and ranges. The "x" shows while a filter is set.

## 0.7.3 - every purchase tied to the right item

### Marketplace matches

- **Purchases and sales are matched against each item's whole history**, not
  only against what is in your inventory today. An item keeps its purchase
  through stickers, name tags, slabs, storage units and trades: a Buff163 or
  CSFloat purchase of a skin you have since stickered is found again, and so is
  a sticker you stuck onto a skin or put into a storage unit. The cause: Steam
  gives an item a new asset id every time it is modified or traded, and each of
  those was treated as a new item, so the purchase stayed with an id that no
  longer existed and read "Item not here yet".
- **A purchase whose item is gone says where it went**: "Applied to AK-47 |
  Inheritance", "Traded away", "In a storage unit", "Listed or sold on the Steam
  market" - instead of "Item not here yet".
- **Each item takes one purchase and one sale.** Several purchases could land
  on the same item before, one per sync; a sticker carried five. The cause: the
  matcher remembered only what it had matched in the same run.
- **Purchase dates come from the item itself**, not from an older item of the
  same kind. Stickers bought in May 2026 showed a date in August 2025, and
  their CSFloat purchase could not be found because it was looked for around
  the wrong date.
- **A failed CSFloat purchase is no longer matched.** It never arrived, and it
  took the item a successful purchase of the same skin needed.
- All matches are made again once after this update, offline, and your
  CSFloat and Buff163 history is read once in full: the first read had dropped
  trades of items that were no longer in the inventory, such as a CSFloat sale.

### Storage units

- **Storage units count what is really inside.** Steam never records taking an
  item out of a unit, so every item that went in, came out and went in again
  was counted twice, and items taken out and traded away stayed in the unit.
  On the account this was measured on, 4,300 became 3,979, against 3,978
  counted by hand.

### Holdings

- **An item you traded away or stuck onto another item is no longer
  "Missing"** when the history shows it: it says "Traded away" or "Applied /
  used up".
